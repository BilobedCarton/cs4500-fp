...
void testDataFrame() {
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		assert(t.doubleAlmostEqual(ds.get_double(1, i), i * -0.1, 3));
	...
	assert(t.doubleAlmostEqual(r2.get_double(1), -0.4, 3));
...
void testColumnFunctionality() {
	...
	// check original data
	assert(ic->get(4) == 10);
	assert(t.doubleAlmostEqual(fc->get(2), 100.9, 3));
...
int main(int argc, char** argv) {
	Test t;
    testDataFrame();
	t.OK("DataFrame tests -- passed.");

    testSchemaFunctionality();
    testRowSemantics();
    testColumnFunctionality();
    testDataframeFunctionality();
	t.OK("DataFrame component functionality tests -- passed.");
